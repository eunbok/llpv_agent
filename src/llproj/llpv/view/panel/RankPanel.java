...
public class RankPanel extends JPanel {
  public RankPanel(Database db) {
    ...
    viewComboBox
        .setModel(new DefaultComboBoxModel<>(MessageUt.getMessage("rank.view_names").split(",")));
    ...
    for (int i = 0; i < data_arr.size(); i++) {
      ...
      lists[i].setText(
          i + 1 + ". " + dv.getRun_file() + " " + "(" + CmnUt.secToTime(dv.getTotal_cnt()) + ")");
    }

    timeComboBox.setFont(CmnVal.font);
    timeComboBox
        .setModel(new DefaultComboBoxModel<>(MessageUt.getMessage("rank.time_names").split(",")));
